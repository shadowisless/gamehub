    ...
    <div class="container">
        ...
        <div style="display:grid; grid-template-columns: 2fr 1fr; gap:2rem; align-items:start;" class="game-detail-grid">
            ...
            <div>
                ...
                <div style="color:var(--text-secondary); margin-bottom:1.5rem; display:flex; flex-wrap:wrap; gap:0.5rem; align-items:center;">
                ...
                <div class="card card-body">
                    ...
                    <div class="table-responsive">
                        <table class="table">
                            ...
                            <tr>
                                <td style="font-weight:600; color:var(--text-primary); width:35%; white-space:nowrap;">{{ ucfirst($key) }}</td>
                                <td>{{ $val }}</td>
                            </tr>
                            @endforeach
                        </table>
            ...
            {{-- Kanan: Action Card --}}
            <div style="position:sticky; top:90px;" class="game-action-card">
                <div class="card card-body">
                    ...
                    <div style="border-top:1px solid var(--border); margin-top:1.25rem; padding-top:1.25rem;">
                        <div style="display:flex; flex-direction:column; gap:0.5rem; font-size:0.85rem; color:var(--text-secondary);">
                            <div class="flex justify-between">
                                <span>Developer</span>
                                <span style="color:var(--text-primary); text-align:right; max-width:60%;">{{ $game->developer }}</span>
                            </div>
                            <div class="flex justify-between">
                                <span>Publisher</span>
                                <span style="color:var(--text-primary); text-align:right; max-width:60%;">{{ $game->publisher }}</span>
                            </div>
                            <div class="flex justify-between">
                                <span>Genre</span>
        ...
        <div class="mt-4">
            ...
            @if($reviews->count() > 0)
            <div style="display:flex; align-items:center; gap:1rem; margin-bottom:1.5rem; padding:1.25rem; background:var(--bg-card); border:1px solid var(--border); border-radius:var(--radius-lg); flex-wrap:wrap;">
                <div style="font-family:var(--font-display); font-size:3.5rem; font-weight:700; color:var(--accent); line-height:1;">{{ $avgRating }}</div>
                <div>
            ...
            <div class="card card-body mb-2">
                <div style="display:flex; align-items:center; gap:0.75rem; margin-bottom:0.75rem; flex-wrap:wrap;">
                    ...
                    <div style="font-size:0.8rem; color:var(--text-muted);">{{ $review->created_at->diffForHumans() }}</div>
                    @auth
                ...
                @if($review->title)
                <div style="font-weight:600; color:var(--text-primary); margin-bottom:0.3rem;">{{ $review->title }}</div>
                @endif
...
labels.forEach((label, index) => {
    label.addEventListener('mouseover', () => {
        labels.forEach((l, i) => { l.style.color = i <= index ? '#ffa502' : 'var(--text-muted)'; });
    });
    label.addEventListener('mouseleave', updateStars);
    label.addEventListener('click', () => setTimeout(updateStars, 10));
});
function updateStars() {
    const checked = document.querySelector('.star-input:checked');
    const val = checked ? parseInt(checked.value) : 0;
    labels.forEach((l, i) => { l.style.color = i < val ? '#ffa502' : 'var(--text-muted)'; });
}
updateStars();
</script>
@endpush
