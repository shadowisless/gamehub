        ...
        <div class="grid-2" style="grid-template-columns: 2fr 1fr; gap: 2rem; align-items: start;">
                ...
                <form action="{{ route('checkout.process') }}" method="POST">
                    ...
                    <div class="card card-body mb-3">
                        ...
                        <div style="display: flex; gap: 1rem; margin-bottom: 1rem;">
                            <label style="flex: 1; display: flex; flex-direction: column; align-items: center; padding: 1.25rem; border: 2px solid var(--border); border-radius: var(--radius); cursor: pointer; transition: var(--transition);" class="purchase-label">
                            ...
                            <label style="flex: 1; display: flex; flex-direction: column; align-items: center; padding: 1.25rem; border: 2px solid var(--border); border-radius: var(--radius); cursor: pointer; transition: var(--transition);" class="gift-label">
                    ...
                    <div class="card card-body mb-3">
                        ...
                        <div style="margin-top: 1rem; padding: 0.75rem 1rem; background: var(--bg-secondary); border-radius: var(--radius); font-size: 0.85rem; color: var(--text-secondary);">
                            ...
                            Ini adalah simulasi pembayaran untuk keperluan UAS. Transaksi akan langsung diproses.
                        </div>
                    </div>

                    <button type="submit" class="btn btn-primary btn-block btn-lg"
                            onclick="return confirm('Konfirmasi pembelian?')">
                        <i class="fa-solid fa-check"></i> Konfirmasi Pembelian — Rp {{ number_format($total, 0, ',', '.') }}
                    </button>
                </form>
            </div>
            ...
            <div class="card card-body" style="position: sticky; top: 90px;">
                ...
                <div class="flex justify-between mb-2" style="font-size: 0.85rem; color: var(--text-secondary);">
                    <span>{{ Str::limit($item['game']->title, 25) }}</span>
                    <span>{{ $item['game']->formatted_price }}</span>
                </div>
                @endforeach
